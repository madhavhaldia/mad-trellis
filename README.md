...
mad-trellis is a **governance substrate for parallel agentic development**. Run many coding agents
...
mad-trellis ships as a single static, cgo-free Go binary (an arbiter daemon + a CLI + a transparent
...
<!-- TODO: demo gif (mad-trellis watch during a 4-agent run) -->
...
```
              ...
              submit / promote (validated, lease-gated)    | read-only
                            v                              |
                  +-----------------------+        +----------------------+
                  |   THE integrator      |        |  mad-trellis watch |
...
`mad-trellis watch` is a seventh terminal: a read-only mirror of trunk tip, in-flight integrations,
...
isn't forking; it's **convergence under concurrency without trust.** mad-trellis is the part a script
doesn't have:
...
- **A falsifiable safety gate.** `mad-trellis conform` boots a real governed scenario and proves the
...
```sh
go install github.com/madhavhaldia/mad-trellis/cmd/mad-trellis@latest
```
...
mad-trellis doctor      # verify the install (paths, git floor, version pins)
```
...
mad-trellis is **per-repo and zero-config**: each git repo automatically gets its own isolated
runtime (socket / ledger / mediated trunk) under `~/.mad-trellis/repos/<hash>/`. Just `cd` into a
...
# on clean exit, the work is auto-converged back. The agent is opaque to mad-trellis:
mad-trellis launch -- claude        # or: codex, a shell, your own tool

# Run several at once — each gets its own boundary; none can see another's edits:
mad-trellis launch -- claude        # (in another terminal)
mad-trellis launch -- codex         # (in a third)

# Watch the live governance loop (read-only):
mad-trellis watch
```

`mad-trellis launch` is **fail-closed**: if it cannot establish governance it refuses to run the
agent (it never runs it ungoverned). On clean exit it tears the boundary down and converges.

**Optional, not required:**

```sh
mad-trellis init          # write mad-trellis.json to declare what's forkable/convergent/singular
mad-trellis daemon        # start the arbiter by hand (launch auto-starts it otherwise)
mad-trellis spawn         # just open an isolated worktree to work in yourself (no agent)
```
...
- **`worktree`** (default, **cross-platform**) — a git worktree per agent, outside the repo, with a
  ...
  (mad-trellis never *hands out* a path that escapes the boundary).
...
## Safety: `mad-trellis conform`

`mad-trellis conform` is the executable safety authority — and it is **falsifiable**. It boots a
...
```sh
mad-trellis conform   # exit 0 = GREEN = safe to self-host; non-zero = RED
```
...
**Early but functional and self-hosting** — mad-trellis develops itself through its own governed
...
> You do **not** need this to use mad-trellis. The Quickstart above is the everyday path. This
...
compare-and-swap (Inv 7). The mediated trunk is a bare repo mad-trellis owns; a receive hook lets
...
git -C ~/.mad-trellis/trunk.git fetch <repo> +<branch>:refs/heads/trunk

# 3. Add the mediated repo as a remote named `med` in your working checkout:
git -C <repo> remote add med ~/.mad-trellis/trunk.git
```

**The loop** (every change), now that `med` exists:

```sh
git checkout -b nm/<slug>                         # isolate the work (or: mad-trellis spawn)
# ...edit, commit on nm/<slug>...
git push med nm/<slug>                            # publish to the mediated trunk (hook allows nm/*)
id=$(mad-trellis trunk submit refs/heads/nm/<slug>)  # -> integration id (received)
mad-trellis trunk promote "$id"                      # validate + lease + atomic advance -> promoted
